...
public class DashboardPresenter implements Presenter<MvpView> {
    ...
    public void deleteAllFromCart(DashBoardActivity activity) {
        ...
        mRepository.deleteAllCart().subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribeWith(new DefaultApiObserver<BaseResponse>(activity) {
            ...
            public void onResponse(BaseResponse response) {
                ...
                activity.onSuccess(response, AppConstants.DELETE_CART);
            }

            @Override
            public void onError(Throwable call, BaseResponse baseResponse) {
                activity.hideProgress();
                activity.onError(call, AppConstants.DELETE_CART);
            }
        });
    }
    ...
    public void estimatedDelivery(DashBoardActivity activity, MerchantRequest request) {
        mView.showProgress();
        mRepository.estimatedDelivery(request).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).
                subscribeWith(new DefaultApiObserver<DeliveryData>(activity) {
                    @Override
                    public void onResponse(DeliveryData response) {
                        mView.hideProgress();
                        mView.onSuccess(response, 1);
                    }
